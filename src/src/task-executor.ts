...
function buildTaskMessage(
  ...
  const deliverableMetadataRules = [
    ...
    '}',
    "```",
    "Technology-specific OpenAPI setup (MANDATORY for all REST API projects):",
    "- **Java Spring Boot**: Add `org.springdoc:springdoc-openapi-starter-webmvc-ui:2.8.6` dependency → auto-generates `/swagger-ui/index.html`",
    "- **Node.js Express**: Add `swagger-ui-express` + `swagger-jsdoc` → mount at `/api-docs`; set `previewReadyPath: \"/api-docs\"`",
    "- **Python FastAPI**: Built-in at `/docs` — set `previewReadyPath: \"/docs\"`",
    "- **Python Flask**: Add `flask-restx` or `flasgger` → mount at `/apidocs`; set `previewReadyPath: \"/apidocs\"`",
    "- **Go (Gin/Echo)**: Add `swaggo/swag` + `gin-swagger` → mount at `/swagger/index.html`; set `previewReadyPath: \"/swagger/index.html\"`",
